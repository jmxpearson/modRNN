...
class AccumulationTaskDataset(Dataset):
    ...
    def _generate_poisson_towers(self, rate: float, length: float) -> np.ndarray:
...
def create_dataloaders(
    ...
    **dataset_kwargs,
    ...
    val_dataset = AccumulationTaskDataset(n_trials=val_trials, seed=1, **dataset_kwargs)
...
def plot_trials(
    ...
    for ax_idx, trial_idx in enumerate(trial_indices):
        ...
        im = axes[ax_idx].imshow(
            data, aspect="auto", cmap="RdYlGn", interpolation="nearest", vmin=-1, vmax=1
        )
        ...
        title = (
            f"Trial {trial_idx}: Rewarded={rewarded_side_str} | "
...
def plot_trial_detailed(
    ...
    for i, (name, color) in enumerate(zip(feature_names[:4], colors[:4])):
        ...
        axes[i].axvline(
            x=cue_duration, color="black", linestyle="--", linewidth=1.5, alpha=0.5
        )

        # Shade cue and delay regions
        axes[i].axvspan(0, cue_duration, alpha=0.1, color="yellow", label="Cue Period")
        axes[i].axvspan(
            cue_duration, time_axis[-1], alpha=0.1, color="cyan", label="Delay Period"
        )
    ...
    axes[4].fill_between(time_axis, 0, target_np, color=colors[4], alpha=0.3)
...
def plot_psychometric_curve(
    dataset: AccumulationTaskDataset, figsize: Tuple[int, int] = (10, 6)
):
    """
    ...
    errors = [1.96 * np.sqrt(p * (1 - p) / n) for p, n in zip(prop_correct, counts)]
