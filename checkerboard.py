...
class DelayedMatchToEvidenceDataset(Dataset):
    ...
    def _sample_coherence(self) -> float:
        ...
        coherence = (
            coherence * (self.coherence_max - self.coherence_min) + self.coherence_min
        )
        coherence = np.clip(coherence, self.coherence_min, self.coherence_max)
        return coherence

    def _generate_fixed_checkers(
        self, coherence: float, predom_color: int, n_channels: int
    ) -> np.ndarray:
        """
    ...
    def _generate_single_trial(self) -> Dict[str, Any]:
        ...
        test_side_cue = np.zeros(
            n_timesteps
        )  # Test side signal (-1 or +1 during test, 0 otherwise)
        # Binary checkerboard inputs - shown during sample period only (-1=black, +1=white, 0=off)
        checkerboard_samples = np.zeros(
            (n_timesteps, self.n_checkerboard_channels), dtype=int
        )
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        cues = np.stack(
            [trial["sample_cue"], trial["delay_cue"], trial["test_cue"], test_side_cue],
            axis=1,
        )

        # Combine cues with checkerboard samples (convert to float for consistency)
        inputs = np.concatenate(
            [cues, trial["checkerboard_samples"].astype(float)], axis=1
        )
    ...
    def get_trial_info(self, idx: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "empirical_predom_color": "white"
            if trial["empirical_predom_color"] == 1
            else "black",
        }
...
def create_dataloaders(
    ...
    **dataset_kwargs,
    ...
    train_dataset = DelayedMatchToEvidenceDataset(
        n_trials=train_trials, seed=0, **dataset_kwargs
    )
    val_dataset = DelayedMatchToEvidenceDataset(
        n_trials=val_trials, seed=1, **dataset_kwargs
    )
    test_dataset = DelayedMatchToEvidenceDataset(
        n_trials=test_trials, seed=2, **dataset_kwargs
    )
...
def plot_trials(
    ...
    for ax_idx, trial_idx in enumerate(trial_indices):
        ...
        delay_end_idx = int(
            (info["sample_duration"] + info["delay_duration"]) / dataset.time_bin_size
        )
        ...
        delay_end = (
            info["sample_duration"] + info["delay_duration"]
        ) / dataset.time_bin_size
...
def plot_trial_detailed(
    ...
    for i in range(3):
        ...
        axes[i].fill_between(time_axis, 0, inputs_np[:, i], color=color, alpha=0.3)
        ...
        # Add vertical lines for epoch boundaries
        axes[i].axvline(
            x=sample_end, color="black", linestyle="--", linewidth=1.5, alpha=0.5
        )
        axes[i].axvline(
            x=delay_end, color="purple", linestyle="--", linewidth=1.5, alpha=0.5
        )
    ...
    for i in range(n_channels_to_plot):
        ...
        axes[ax_idx].plot(
            time_axis, inputs_np[:, channel_idx], color=colors[ax_idx], linewidth=2
        )
        axes[ax_idx].fill_between(
            time_axis, 0, inputs_np[:, channel_idx], color=colors[ax_idx], alpha=0.3
        )
        axes[ax_idx].set_ylabel(f"Checker {i}", fontsize=11, fontweight="bold")
        axes[ax_idx].set_ylim(-1.2, 1.2)
        axes[ax_idx].axhline(
            y=0, color="black", linestyle="-", linewidth=0.5, alpha=0.3
        )
    ...
    axes[5].fill_between(time_axis, 0, side_cue_np, color=colors[5], alpha=0.3)
    ...
    axes[6].fill_between(time_axis, 0, target_np, color=colors[6], alpha=0.3)
...
def plot_psychometric_curve(
    dataset: DelayedMatchToEvidenceDataset, figsize: Tuple[int, int] = (12, 5)
):
    """
    ...
    for bin_idx in sorted(coherence_data.keys()):
        if coherence_data[bin_idx]:
            coh_vals.append(
                coherence_bins[bin_idx - 1] if bin_idx > 0 else coherence_bins[0]
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Number of input features: 3 cues + {dataset.n_checkerboard_channels} checkerboard channels = {inputs.shape[1]}"
    )
    print(
        f"Target values (first 5, last 5): {target[:5].numpy()}, {target[-5:].numpy()}"
    )
    ...
    if "empirical_coherence" in info:
        ...
        print("These are binary (0=black, 1=white) and FIXED throughout sample period")
        print(
            f"Empirical coherence: {info['empirical_coherence']:.2f} (target: {info['coherence']:.2f})"
        )
        print(
            f"Number of predominant-color checkers: {info['n_predom_checkers']}/{dataset.n_checkerboard_channels}"
        )
    ...
    train_loader, val_loader, test_loader = create_dataloaders(
        train_trials=800, val_trials=100, test_trials=100, batch_size=32
    )
